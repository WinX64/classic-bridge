package dev.winx64.classicbridge.classic.network.clientbound

import dev.winx64.classicbridge.classic.cpe.ExtensionSettings
import dev.winx64.classicbridge.classic.network.ClassicInboundPacket
import dev.winx64.classicbridge.classic.network.callback.ClassicPacketCallback
import dev.winx64.classicbridge.classic.network.readByteArray
import io.netty.buffer.ByteBuf

class LevelDataChunkClientboundPacket(input: ByteBuf, settings: ExtensionSettings) : ClassicInboundPacket {

    val chunkLength = input.readShort()
    val chunkData = input.readByteArray()
    val percentComplete = input.readByte()

    override fun notifyCallback(callback: ClassicPacketCallback) = callback.onLevelDataChunk(this)
}
